...
export class DALLE2Provider {
  ...
  async _convertImageToPNG(imageBuffer) {
    // DALL-E 2 variation API requires PNG format
    // For simplicity, we'll assume the reference image is already in a compatible format
    // In production, you might want to add image conversion logic here
  ...
  async _generateVariationFromReference(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const referenceImageBuffer = await this._downloadImage(
          referenceImageUrl
        );
        const pngBuffer = await this._convertImageToPNG(referenceImageBuffer);
