...
contract NFTCollection is ERC721URIStorage, ReentrancyGuard, Ownable {
    ...
    event TokenMinted(uint256 indexed tokenId, address indexed minter, string prompt, uint256 timestamp);
    event GenerationRequested(uint256 indexed tokenId, string prompt, address requester);
    ...
    function mint(string memory prompt) external payable nonReentrant mintingAllowed {
    ...
    function updateTokenURI(uint256 tokenId, string memory newTokenURI) external {
        require(msg.sender == factory || msg.sender == owner(), "Not authorized to update URI");
